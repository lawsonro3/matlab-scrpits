function [powe,fre,total]=fast_psd(x,nfft_in,fs)
% function [p,f]=fast_psd(x,nfft,Fs,trendtype) computes the properly 
...
% segments are fit so that all of the data is used. The trend type for the
% detrend function must be specified
max_ind=length(x);
